...
public interface ProcessInspector {
	...
	/**
	 * When it bypasses an alternative element, it will be not added to the result
	 * list.
	 */
	public void disableDescribeAlternativeElements();

	/**
	 * Return a list of all tasks in the process which can be reached from the
	 * starting element.
	 * 
	 ...
	 * starting element.
	 * 
	 * @param startAtElements - Elements where we start traversing the process. In
	 *                        case of parallel tasks, the list will contain multiple
	 *                        objects.
	 * @param useCase         - Use case that should be used to read duration
	 *                        values. Durations will be set to 0 in case not
	 *                        provided.
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Return a list of all tasks in the process which can be reached from active
	 * case.
	 * 
	...
	/**
	 * Return a list of all tasks which are created when process follows the tagged
	 * flow. Uses the flow name set in the constructor.
	 * 
	 * @param startAtElement - Element where we start traversing the process
	 * @param useCase        - Use case that should be used to read duration values.
	 *                       Durations will be set to 0 in case not provided.
	...
	/**
	 * This method can be used to override configured path taken after an
	 * alternative gateway.
	 * 
	 * @param processFlowOverrides key: alternative element PID value: chosen output
	 *                             PID
	 * @return
	 */
	public ProcessInspector setProcessFlowOverrides(Map<String, String> processFlowOverrides);

	/**
	 * This method can be used to override configured task duration of the model by
	 * own values.
	 * 
	 * @param durationOverrides key: ElementTask - with
	 *                          ElementTask.createGateway(element ID, task
	 *                          identifier) for TaskSwitchGateway,
	 *                          ElementTask.createSingle(element ID) for other
	 *                          value: new duration
	 * @return
	 */
	public ProcessInspector setDurationOverrides(Map<ElementTask, Duration> durationOverrides);
}